...
export default function WalletGraphic({ statisticArray }) {
  const coins = statisticArray.map((coin, index) => (
    <li className="flex 3xl:gap-x-0 gap-x-24 justify-between items-center" key={index}>
      <div className="flex items-center gap-x-4">
        <img className="!w-10 !h-10 object-contain" src={coin.iconUrl} alt={coin.title} />
        <span className="text-xl font-semibold text-white">{coin.title}</span>
        <span className="font-inter px-[2px] text-[10px] font-semibold text-notr border-[1px] border-notr rounded-[3px]">9.9%</span>
      </div>
      <span className="text-xl font-semibold text-white">{coin.value}</span>
    </li>
  ));
  ...
  return (
    <div className="w-full flex 3xl:flex-row 3xl:justify-between items-center flex-col gap-y-12">
      ...
      <div className="3xl:self-auto w-full 3xl:w-1/2 px-4 sm:px-16 lg:px-4">
